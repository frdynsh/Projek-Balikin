<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            {{-- Tombol ini mengarah ke route 'barang-temuan.create' --}}
            <a href="{{ route('barang-temuan.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                Lapor Penemuan
            </a>
        </div>
    </x-slot>
                ...
                @forelse ($barangTemuans as $barang)
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg flex flex-col">
                        ...
                        <div class="p-6 flex flex-col flex-grow">
                            ...
                            {{-- Menggunakan kolom 'tgl_penemuan' dan 'lokasi_penemuan' dari database --}}
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Ditemukan pada: {{ \Carbon\Carbon::parse($barang->tgl_penemuan)->isoFormat('D MMMM YYYY') }}</p>
                            <p class="text-sm text-gray-600 dark:text-gray-400">Lokasi: {{ $barang->lokasi_penemuan }}</p>
                            <p class="mt-2 text-gray-800 dark:text-gray-200 flex-grow">{{ Str::limit($barang->deskripsi_barang, 100) }}</p>
                            {{-- Mengambil nama pelapor dari relasi 'user' yang sudah dibuat --}}
